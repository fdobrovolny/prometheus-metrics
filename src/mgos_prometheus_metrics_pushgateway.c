...
static char *mgos_prometheus_post_uri() {
  ...
  if (s_instance) {
    snprintf(s_uri, sizeof(s_uri), "/metrics/job/%s/instance/%s", s_job,
             s_instance);
  } else {
...
static void mgos_prometheus_metrics_post_finish(struct mg_connection *nc) {
  ...
  LOG(LL_ERROR,
      ("POST failed, response follows: %.*s", (int) io->len, io->buf));
}

static void mgos_prometheus_metrics_post_handler(struct mg_connection *nc,
                                                 int ev, void *ev_data,
                                                 void *user_data) {
  switch (ev) {
    case MG_EV_CONNECT: {
...
void mgos_prometheus_metrics_push(const char *job, const char *instance) {
  if (!job) {
    LOG(LL_ERROR,
        ("Need to set 'job' argument in order to POST to pushgateway at %s",
         mgos_sys_config_get_prometheus_pushgateway()));
  }
  if (s_job) {
    free(s_job);
  ...
  mg_connect(mgos_get_mgr(), mgos_sys_config_get_prometheus_pushgateway(),
             mgos_prometheus_metrics_post_handler, NULL);
